...
public class ElevensBoard extends Board {
	...
	private ArrayList<Integer> findJQK(List<Integer> selectedCards) {
		...
		ArrayList<Integer> returnCards = new ArrayList<Integer>(), empty = new ArrayList<Integer>();
		for (Integer kObj : selectedCards) {
			int i = kObj.intValue();
			if (cardAt(i).rank().equals("jack") && !j) {
				j = true;
				returnCards.add(i);
			} else if (cardAt(i).rank().equals("queen") && !q) {
				q = true;
				returnCards.add(i);
			} else if (cardAt(i).rank().equals("king") && !k) {
				k = true;
				returnCards.add(i);
			}
		}
		return (j && q && k) ? returnCards : empty;
	}
	...
	public boolean playIfPossible() {
		return (playPairSum11IfPossible(findPairSum11(cardIndexes())) || playJQKIfPossible(findJQK(cardIndexes())));
	}
	 ...
	 */
	private boolean playPairSum11IfPossible(ArrayList<Integer> cards) {
		if(cards.size() > 0) {
	 ...
	 */
	private boolean playJQKIfPossible(ArrayList<Integer> cards) {
		if (cards.size() > 0) {
			replaceSelectedCards(cards);
			return true;
		}
